...
import { exportToLeRobot, generateLeRobotDataLines, generateLeRobotStructure } from '@/lib/tlabel/lerobot-exporter';

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { fileName, fileContent, options } = body;
    ...
    try {
      ...
      return NextResponse.json({ error: '数据解析失败', steps }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: 'TLabel转换失败', steps }, { status: 500 });
    }
